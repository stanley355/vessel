...
const AwaitingPaymentForm = (props: IAwaitingPayment) => {
  ...
  const RenewPaymentBtn = () => (
    <div className={styles.renew}>
      <div className={styles.renew__info}>Tagihan Anda sudah tidak berlaku, klik di bawah untuk memperbaharui tagihan</div>
      <button type='button' onClick={onRenewClick}>Perbarui Langganan</button>
    </div>
  )
  ...
  return (
    <div className={styles.await__payment}>
      ...
      <div className={styles.subtitle}>Menunggu pembayaran untuk langganan Channel {channelName}</div>

      <form onSubmit={() => { }}>
        <div className={styles.info}>Nama pelanggan : {profile.fullname} </div>
        <div className={styles.info}>Email : {profile.email} </div>
        <div className={styles.info}>Durasi Langganan: {subscriptionDuration}</div>
